...
contract ExchangeCore is ReentrancyGuarded, Ownable {
        ...
        returns (bool)
    {
        ...
        // order.exchangeはこのコントラクトアドレスである必要あり
        ...
        returns (uint)
    {
        ...
        //　paymentTokenのaddress指定がある時は、Etherの送金は受け付けない
        ...
        // ERC20トークンでの支払いの場合、ここで buy.makerからsell.makerにpriceを送金 
        if (price > 0 && sell.paymentToken != address(0)) {
            // buy.makerからsell.makerにpriceを移動
        ...
        if (sell.feeRecipient != address(0)) { // sell.feeRecipientが設定されている時は、sell side order が maker
            ...
            if (sell.feeMethod == FeeMethod.SplitFee) {
                ...
                if (sell.makerRelayerFee > 0) {
                    ...
                    if (sell.paymentToken == address(0)) { //Etherの場合
            ...
            } else {// ProtocolFee: ERC20トークンの支払いが前提
        ...
        } else { // sell.feeRecipientが設定されていない場合は、buy side order が maker
        ...
        // Etherでの支払いの場合、ここで sell.makerにreceiveAmountを送金
        if (sell.paymentToken == address(0)) {
        ...
        reentrancyGuard
    {
        ...
        uint size;
        address target = sell.target; // NFT address
        assembly {
